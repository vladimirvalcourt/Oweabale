...
export function applyTransferSafetySettings() {
  console.log('[Transfer Safety] Applying conservative egress protection settings');
  console.log('[Transfer Safety] Transaction limit:', TRANSFER_SAFETY_CONFIG.MAX_TRANSACTIONS_PER_PAGE);
  console.log('[Transfer Safety] Phase 2 limit:', TRANSFER_SAFETY_CONFIG.MAX_PHASE2_RECORDS);
  console.log('[Transfer Safety] Data freshness:', TRANSFER_SAFETY_CONFIG.DATA_FRESHNESS_THRESHOLD_MS / 60000, 'minutes');
  console.log('[Transfer Safety] Max concurrent requests:', TRANSFER_SAFETY_CONFIG.MAX_CONCURRENT_REQUESTS);
...
export function disableTransferSafetyMode() {
  ...
  console.log('[Transfer Safety] Transfer safety mode disabled');
}
