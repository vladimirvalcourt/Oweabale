...
export function resetEgressMetrics() {
  ...
  try {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(metrics));
    console.log('[EgressMonitor] Metrics reset');
  } catch (err) {
...
export function logEgressSummary() {
  ...
  console.group('📊 Supabase Egress Summary');
  console.log(`Total Requests: ${totalRequests}`);
  console.log(`Estimated Usage: ${gbUsed.toFixed(3)} GB / 5 GB`);
  console.log(`Remaining: ${gbRemaining.toFixed(3)} GB`);
  console.log(`\nRequests by Table:`);
  Object.entries(requestsByTable)
    ...
    .forEach(([table, count]) => {
      ...
      console.log(`  ${table}: ${count} requests (~${mb.toFixed(2)} MB)`);
    });
  console.groupEnd();
}
