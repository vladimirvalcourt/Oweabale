          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="bg-surface-raised border border-surface-border rounded-sm overflow-hidden shadow-sm">
              <div className="px-6 py-4 border-b border-surface-border flex items-center justify-between bg-zinc-900/50">
                <h2 className="text-xs font-mono font-bold text-zinc-400 uppercase tracking-widest">Score Breakdown</h2>
            ...
            <section className="bg-indigo-600/5 border border-indigo-500/20 rounded-sm p-8 relative overflow-hidden group">
              ...
              <div className="relative z-10 text-left">
                ...
                {boostTip ? (
                  ...
                  <p className="text-zinc-500 text-sm">Add active credit lines to see high-velocity boost tips.</p>
                )}
              </div>
            </section>
          </div>
          ...
          <div className="space-y-8">
            ...
            <section className="bg-surface-raised border border-surface-border rounded-sm flex flex-col h-full shadow-lg">
              ...
              <div className="flex-1 divide-y divide-surface-border overflow-y-auto max-h-[500px] outline-none">
                {(!credit?.fixes || credit.fixes.length === 0) ? (
                  <div className="p-12 text-center space-y-4">
                    ...
                    <div>
                      <p className="text-xs font-mono text-zinc-500 uppercase tracking-widest">Clear Record</p>
                      <p className="text-[10px] text-zinc-600 mt-1 uppercase tracking-tight">System monitoring active</p>
                    </div>
                  </div>
                ) : (
                    ...
                    <div key={fix.id} className="p-5 space-y-3 hover:bg-white/5 transition-colors text-left group">
                      ...
                      <div className="flex items-center gap-2 pt-1 opacity-80 group-hover:opacity-100 transition-opacity">
                        ...
                        <button 
                          ...
                          className="flex-1 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 border border-white/5 rounded-sm text-[10px] font-mono font-bold uppercase tracking-widest transition-all"
                        >
              ...
              <div className="p-6 bg-surface-base border-t border-surface-border space-y-4">
                <div className="flex items-center gap-2 text-[10px] font-mono font-bold text-zinc-500 uppercase tracking-widest">
                  <ShieldAlert className="w-3.5 h-3.5 text-indigo-500" /> Protection Shield
                </div>
                <div className="space-y-2">
                  <a href="https://www.annualcreditreport.com" target="_blank" rel="noreferrer" className="flex items-center justify-between p-2.5 text-[11px] text-zinc-400 hover:text-white bg-white/5 border border-white/5 rounded-sm transition-all group">
                    Full Credit Report <ExternalLink className="w-3 h-3 opacity-50 group-hover:opacity-100" />
                  </a>
                  <a href="https://www.experian.com" target="_blank" rel="noreferrer" className="flex items-center justify-between p-2.5 text-[11px] text-zinc-400 hover:text-white bg-white/5 border border-white/5 rounded-sm transition-all group">
            ...
            <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
              ...
              >
                ...
                <form onSubmit={handleAddFix} className="space-y-4">
                  <div>
                    <label className="block text-[10px] font-mono font-bold text-zinc-500 uppercase tracking-widest mb-1.5">Description</label>
                    <input 
                      autoFocus
                      type="text" 
                      value={fixItem}
                      ...
                      className="w-full bg-surface-base border border-surface-border rounded-sm px-3 py-3 text-sm text-white focus:border-indigo-500 outline-none placeholder:text-zinc-700"
                    />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[10px] font-mono font-bold text-zinc-500 uppercase tracking-widest mb-1.5">Amount ($)</label>
                      <input 
                        type="number" 
                        value={fixAmount}
                    ...
                    <div>
                      <label className="block text-[10px] font-mono font-bold text-zinc-500 uppercase tracking-widest mb-1.5">Bureau</label>
                      <select 
                        value={fixBureau}
                  ...
                  <div>
                    <label className="block text-[10px] font-mono font-bold text-zinc-500 uppercase tracking-widest mb-1.5">Evidence / Notes</label>
                    <textarea 
                      value={fixNotes}
                      onChange={e => setFixNotes(e.target.value)}
                      placeholder="Explain why this is incorrect..."
                      className="w-full bg-surface-base border border-surface-border rounded-sm px-3 py-3 text-sm text-white focus:border-indigo-500 outline-none h-24 resize-none placeholder:text-zinc-700"
                    />
                  </div>
                  <div className="flex gap-3 pt-4">
                    <button 
                      type="button"
                      onClick={() => setIsFixModalOpen(false)}
                      className="flex-1 py-3 text-xs font-mono font-bold text-zinc-500 hover:text-white uppercase tracking-widest transition-colors"
                    >
                      Dismiss
                    </button>
                    <button 
              ...
              >
                <div className="flex justify-between items-start mb-8 border-b border-zinc-200 pb-4 no-print">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs font-sans text-zinc-500 uppercase tracking-widest">Formal Legal Correspondence</p>
                  </div>
