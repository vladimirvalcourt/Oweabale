        ...
        {hasFullSuite || isAdmin ? (
          <div className="space-y-6">
            {[
              ...
              { id: 'detonator-milestone' as const, label: 'Debt payoff milestones', desc: 'Celebrate when I pay off 25%, 50%, 75%, and 100% of a debt.' },
              { id: 'detonator-rate' as const, label: 'Debt rate changes', desc: 'Alert me if a variable interest rate changes.' },
            ].map((item) => (
              <div key={item.id} className="flex items-start justify-between border-b border-surface-border pb-4 last:border-0 last:pb-0">
                <div className="pr-4">
                  <label htmlFor={item.id} className="text-sm font-medium text-content-primary cursor-pointer">{item.label}</label>
          ...
          <FullSuiteGateCard
            ...
            description="Upgrade to turn on subscription and debt payoff alerts."
          />
        )}
      </CollapsibleModule>
    </div>
