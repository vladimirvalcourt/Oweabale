...
-- Skip all document_capture_sessions operations if the table doesn't exist
...
-- Create storage policies - skip document_capture_sessions checks if table doesn't exist
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables
  ...
  ELSE
    -- Fallback: simpler policy without document_capture_sessions checks
    CREATE POLICY "scans_authenticated_manage_owned_objects"
      ON storage.objects
      FOR ALL
      TO authenticated
