# Oweable Design System: Brutalist Agency Aesthetic

## The Oweable Evolution
While originally inspired by Linear's dark native aesthetic, Oweable has evolved into a "Productive Density" framework. It rejects soft UI trends in favor of high-information density and rigid layout structures.

### The Zero-Radius Policy
- **Global Constraint**: `border-radius: 0px` is applied to all primary containers, buttons, and inputs.
- **Rationale**: To communicate a "no-nonsense," institucional, and professional atmosphere. Roundness is perceived as "friendly/consumer," whereas sharpness is perceived as "precise/professional."

### High-Density Layouts
- **Vertical Space**: Minimize padding between data rows.
- **Strict Hierarchy Rules**: Immediate actionable risks must precede analytical and historical data. (e.g. Dashboard "Action Center").
- **Accessibility Minimums**: Adhere to WCAG 2.2 contrast/typography metrics. Font sizes never fall under `12px` globally to guarantee legibilty.
- **Tabular Data**: Use `JetBrains Mono` with `tabular-nums` for all financial entries to ensure perfect vertical alignment of decimal points.
- **Micro-Copy**: Use clear, actionable language. Replace institutional jargon (e.g., "Obligations", "Ambushes") with user-friendly terms (e.g., "Regular Bills", "Unpaid Tickets").
- **Humanization Protocol**: Strictly enforce natural, financial language over technical jargon. Terms like "Identity Protocol" must be "My Information," and "Active Session" becomes "Personal Dashboard."

### The Multi-Aesthetic Engine
- **Obsidian (Default)**: Deepest black, indigo accents. The core experience.
- **Terminal (Emerald)**: Matrix-inspired, high-contrast neon green.
- **Solarized (Amber)**: Industrial safety colors, warm and authoritative.
- **Nordic (Cyan)**: Arctic-cool, calm and precise.
- **Crimson (Ruby)**: High-urgency, aggressive financial tracking.
- **Mono Ghost**: Minimalist grayscale with subtle light borders.
### The Deterministic Intelligence Core (No AI)
- **Liquidity Sniper**: Algorithmic projection of upcoming bills vs income to predict overdrafts within a 14-day window. Triggered as a high-severity "Siren" alert.
- **Price Pulse Engine**: Comparative analysis of historical subscription costs vs current ledger entries to detect unauthorized price hijacks.
- **Merchant Mapping**: Rule-based categorization engine that uses high-speed string matching to auto-assign transaction classes without probabilistic inference.
- **Avalanche Math**: Pure mathematical prioritization of debt based on APR velocity, ensuring zero-slop financial efficiency.

---

# Design System Foundation: Linear (Standard Reference)
...
| Label | Inter Variable | 12px (0.75rem) | 400–590 | 1.40 | normal | Default standard tiny label. Below 12px is prohibited |
...
- Padding: 1px 6px
- Radius: 5px
- Focus state: Outline explicit ring-2 indicators (`focus-visible:ring-indigo-500`) with high contrast errors to prevent destructive UX loops.
