...
- **`browserSupportsModernWebCrypto()`** (`src/lib/browserSupport.ts`) — requires `crypto.subtle` and `getRandomValues` (needed for PKCE). If missing, an **Unsupported browser** banner is shown (`UnsupportedBrowserBanner`).
